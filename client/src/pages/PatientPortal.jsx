...
const patientShareUrl = token 
  ...
  useEffect(() => {
    const initPlan = async () => {
      try {
        const localPlan = await getLatestPlan(token);
  ...
  const toggleExercise = async (exerciseId) => {
    ...
    if (plan && completedExerciseIds.length >= plan.exercises.length) {
      try {
        ...
        console.warn(
          "Unable to sync completed plan yet:",
          error.response?.data?.message || error.message,
        );
      }
    }
  ...
  const submitDoctorRating = async () => {
    ...
    try {
      ...
      setRatingMessage(
        "Thank you. Your rating has been sent to the doctor profile.",
      );
    } catch (error) {
      setRatingMessage(
        error.response?.data?.message || "Unable to submit rating right now.",
      );
    }
  };
  ...
  const stats = useMemo(() => {
    ...
    const dailyPercent = Math.round(
      (dailyLog.completedExerciseIds.length / totalExercises) * 100,
    );
    const startDate = new Date(plan.createdAt || Date.now());
    const totalDays = Math.max(Number(plan.durationWeeks || 1) * 7, 1);
    const day = Math.min(
      Math.floor((new Date() - startDate) / 86400000) + 1,
      totalDays,
    );
    const timelinePercent = Math.round((day / totalDays) * 100);
    const streak = logs.filter(
      (log) => log.completedExerciseIds?.length > 0,
    ).length;
    return { dailyPercent, timelinePercent, day, totalDays, streak };
  }, [dailyLog.completedExerciseIds.length, logs, plan]);

  if (loading)
    return <div className="loading-screen">Syncing your plan...</div>;
  if (!plan)
    return <div className="error-screen">Plan not found or expired.</div>;
      ...
      <header className="patient-hero">
        <div className="hero-topline">
          <div>
            ...
            <h1>
              {text.greeting}, {plan.patientName}
            </h1>
          </div>
        ...
        <div className="recovery-card">
          <div>
            ...
            <strong>
              {stats.dailyPercent}% {text.complete}
            </strong>
          ...
          <div
            ...
            style={{ "--value": `${stats.dailyPercent * 3.6}deg` }}
          >
            <span>
              {dailyLog.completedExerciseIds.length}/{plan.exercises.length}
            </span>
          </div>
        </div>
      ...
      <main className="patient-main">
        <section className="section-heading">
          ...
          <button
            ...
            onClick={() => navigate(`/history/${token}`)}
          >
            <Activity size={18} />
            {text.progress}
          </button>
        ...
        <div className="exercise-stack">
          {plan.exercises.map((exercise, index) => {
            const completed = dailyLog.completedExerciseIds.includes(
              exercise.id,
            );
              ...
              >
                ...
                >
                  ...
                  <div className="exercise-copy">
                    ...
                    <p>
                      {exercise.sets} {text.sets} x {exercise.reps} {text.reps}
                    </p>
                  </div>
                  <ArrowRight size={20} />
                </button>
        ...
        <ContactDoctorButton
          doctorName={
            plan.doctorName || plan.therapistName || "PhysioPath Therapist"
          }
        ...
        <section className="doctor-rating-card">
          ...
          <div className="doctor-rating-actions">
            <div
              ...
              aria-label="Rate doctor from 1 to 5 stars"
            >
